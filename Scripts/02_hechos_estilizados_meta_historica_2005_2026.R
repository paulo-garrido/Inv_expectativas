...
# ---------------------------
# 4. Definir meta histórica de inflación
# ---------------------------
# Fuente: Banco de Guatemala. Serie histórica del EMEI.
# Valor central de la meta y rango de tolerancia de +/- 1 p.p.:
# 2005:       5.0%
# 2006:       6.0%
# 2007:       5.0%
# 2008-2009:  5.5%
...
# 2012:       4.5%
# Desde 2013: 4.0%
# Antes de 2005 se deja NA porque el gráfico histórico busca representar
# específicamente las metas bajo el EMEI.
...
# Verificación rápida de la secuencia histórica de metas
stopifnot(
  all(bd$meta_central[year(bd$fecha) == 2005] == 5.0, na.rm = TRUE),
  all(bd$meta_central[year(bd$fecha) == 2006] == 6.0, na.rm = TRUE),
  all(bd$meta_central[year(bd$fecha) == 2007] == 5.0, na.rm = TRUE),
  all(bd$meta_central[year(bd$fecha) %in% c(2008, 2009)] == 5.5, na.rm = TRUE),
  all(bd$meta_central[year(bd$fecha) %in% c(2010, 2011)] == 5.0, na.rm = TRUE),
  all(bd$meta_central[year(bd$fecha) == 2012] == 4.5, na.rm = TRUE),
  all(bd$meta_central[year(bd$fecha) >= 2013] == 4.0, na.rm = TRUE)
)
...
bd <- bd %>%
  mutate(
    inf_dentro_banda    = ifelse(!is.na(inf) & inf >= banda_inf & inf <= banda_sup, 1, 0),
    exp12_dentro_banda  = ifelse(!is.na(exp_12) & exp_12 >= banda_inf & exp_12 <= banda_sup, 1, 0),
    exp24_dentro_banda  = ifelse(!is.na(exp_24) & exp_24 >= banda_inf & exp_24 <= banda_sup, 1, 0)
  )
...
g1 <- ggplot(bd_plot, aes(x = fecha)) +
  geom_ribbon(aes(ymin = banda_inf, ymax = banda_sup), alpha = 0.20) +
  geom_line(aes(y = inf, color = "Inflación observada"), linewidth = 1) +
  geom_line(aes(y = meta_central, color = "Meta central"), linetype = "dashed", linewidth = 0.9) +
...
g8 <- bd_plot %>%
  ...
  geom_line(linewidth = 1) +
  geom_line(data = bd_plot, aes(x = fecha, y = meta_central),
...
if ("Tasa_pol" %in% names(bd_plot)) {
  g9 <- ggplot(bd_plot, aes(x = fecha)) +
    ...
    geom_line(aes(y = meta_central, color = "Meta"), linetype = "dashed", linewidth = 0.9) +
    scale_y_continuous(labels = label_number(suffix = "%")) +
    labs(
...
# Asignar directamente la meta histórica a toda la serie.
# Esto permite mostrar correctamente el rango meta desde 2005, aunque la
# base principal de expectativas comience posteriormente.
...
g_hist_inf <- ggplot(infl_hist_plot, aes(x = fecha, y = inflacion)) +
  ...
  # Banda de tolerancia desde el inicio del EMEI en 2005
  geom_ribbon(
    data = infl_hist_plot %>% dplyr::filter(fecha >= fecha_emei, !is.na(banda_inf), !is.na(banda_sup)),
    aes(x = fecha, ymin = banda_inf, ymax = banda_sup),
    inherit.aes = FALSE,
    alpha = 0.18,
    fill = "#A9DFBF"
  ) +
  ...
  geom_line(
    data = infl_hist_plot %>% dplyr::filter(fecha >= fecha_emei, !is.na(banda_inf)),
    aes(x = fecha, y = banda_inf),
    inherit.aes = FALSE,
    color = "#0047AB",
    linetype = "dotted",
    linewidth = 0.8
  ...
  geom_line(
    data = infl_hist_plot %>% dplyr::filter(fecha >= fecha_emei, !is.na(banda_sup)),
    aes(x = fecha, y = banda_sup),
    inherit.aes = FALSE,
    color = "#0047AB",
    linetype = "dotted",
    linewidth = 0.8
  ...
  # Meta central
  geom_line(
    data = infl_hist_plot %>% dplyr::filter(fecha >= fecha_emei, !is.na(meta_central)),
    aes(x = fecha, y = meta_central),
    inherit.aes = FALSE,
    color = "#0047AB",
    linetype = "dashed",
    linewidth = 0.9
...
file.exists("figuras y tablas/figuras/fig_inflacion_historica_1997_banda.png")

list.files("figuras y tablas/figuras", full.names = TRUE)

list.files(recursive = TRUE, pattern = "fig_inflacion_historica_1997_banda.png")

# Expectativas de inflación a 12 y 24 meses frente a la meta central, con banda
