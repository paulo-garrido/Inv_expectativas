...
# Si el script generador anterior guardó la base con otro nombre,
# cambia únicamente esta ruta.
...
# ------------------------------------------------------------
# 3) Cargar base ya procesada
# ------------------------------------------------------------
if (!file.exists(ruta_bd_rds)) {
  stop(
    paste0(
      "No se encontró la base procesada en: ", ruta_bd_rds,
      "\nEjecuta primero el script generador de la base o ajusta ruta_bd_rds."
    )
  )
}
...
# ------------------------------------------------------------
# 5) Verificar/reconstruir variables necesarias
# ------------------------------------------------------------
...
if (length(faltantes) > 0) {
  stop(
    paste0(
      "Faltan las siguientes variables en la base procesada: ",
...
bd <- bd %>%
  dplyr::mutate(
    ...
    # Desviaciones con signo respecto de la meta vigente
...
# Indicador descriptivo de estabilidad:
# promedio móvil de 12 meses de la variación mensual absoluta.
# Menor valor = mayor estabilidad de la trayectoria.
bd <- bd %>%
  dplyr::mutate(
...
# ------------------------------------------------------------
# Paleta sobria y consistente con las figuras que hemos trabajado.
col_exp12 <- "#1F3A5F"
col_exp24 <- "#7A8798"
col_meta  <- "#0047AB"
col_banda <- "#EAF2F8"
...
# Las figuras no llevan título interno: el título/caption se controla en Quarto.
...
# Función auxiliar para guardar PNG + PDF vectorial
# (PDF recomendado para insertar en Quarto/PDF).
...
# ============================================================
# FIGURA DE CONTEXTO (NO ES EL RESULTADO PRINCIPAL DEL OBJETIVO 1)
# Evolución histórica de la inflación y metas bajo el EMEI
# Recomendación: ubicarla en la sección 3.
# ============================================================
...
if (file.exists(ruta_inflacion_hist)) {
  ...
  g_contexto <- ggplot2::ggplot(
    ...
    ggplot2::geom_line(
      data = infl_hist %>%
        dplyr::filter(fecha >= fecha_emei, !is.na(meta_central)),
      ggplot2::aes(x = fecha, y = banda_inf),
      inherit.aes = FALSE,
      color = col_borde,
      linetype = "dotted",
      linewidth = 0.5
    ...
    ggplot2::geom_line(
      data = infl_hist %>%
        dplyr::filter(fecha >= fecha_emei, !is.na(meta_central)),
      ggplot2::aes(x = fecha, y = banda_sup),
      inherit.aes = FALSE,
      color = col_borde,
      linetype = "dotted",
      linewidth = 0.5
    ...
    ggplot2::geom_line(
      data = infl_hist %>%
        dplyr::filter(fecha >= fecha_emei, !is.na(meta_central)),
      ggplot2::aes(x = fecha, y = meta_central),
      inherit.aes = FALSE,
      color = col_meta,
      linetype = "dashed",
      linewidth = 0.75
...
# Figura principal de la sección 6.1.
# ============================================================
...
# Desviación con signo + distancia absoluta respecto de la meta
# Dos paneles en una sola figura para evitar redundancia.
# ============================================================

bd_gap_exp <- bd_plot %>%
  dplyr::select(fecha, gap_exp_12, gap_exp_24) %>%
  tidyr::pivot_longer(
  ...
  dplyr::mutate(
    serie = dplyr::recode(
      serie,
      gap_exp_12 = "Expectativa a 12 meses",
      gap_exp_24 = "Expectativa a 24 meses"
    )
  )
...
bd_dist_abs <- bd_plot %>%
  ...
  dplyr::mutate(
    serie = dplyr::recode(
      serie,
      abs_gap_exp_12 = "Expectativa a 12 meses",
      abs_gap_exp_24 = "Expectativa a 24 meses"
    )
  )
...
g_distancia <- ggplot2::ggplot(
  ...
  ggplot2::labs(y = "Distancia respecto a la meta") +
  tema_wp +
  ggplot2::theme(
...
# Promedio móvil de 12 meses de la variación mensual absoluta.
# Menor valor = trayectoria más estable.
# ============================================================
...
g_obj1_estabilidad <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    y = "Variación mensual absoluta promedio"
  ) +
  tema_wp
g_obj1_estabilidad
guardar_figura(
...
resumen_obj1 <- function(data, etiqueta_periodo) {
  tibble::tibble(
    ...
    exp12_dentro_banda = 100 * mean(
      data$exp12_dentro_banda,
      na.rm = TRUE
    ),
    exp24_dentro_banda = 100 * mean(
      data$exp24_dentro_banda,
      na.rm = TRUE
    )
  )
}
...
# ------------------------------------------------------------
# 7) Guardar base enriquecida para el objetivo 1
# ------------------------------------------------------------
saveRDS(
  bd_plot,
  file = file.path(
